...
function easy_storefront_general_admin_notice(){
         ...
         $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" ><p>
				...
				esc_url(EASY_STOREFRONT_THEME_URI),
				esc_html__('Upgrade To Pro','easy-storefront'),
...
if ( isset( $_GET['easy_storefront_admin_notice'] ) ){
	update_option('admin_notice', 4);
}

$easy_storefront_refreshed = isset($_SERVER['HTTP_CACHE_CONTROL']) && ($_SERVER['HTTP_CACHE_CONTROL'] === 'max-age=0' 
||  $_SERVER['HTTP_CACHE_CONTROL'] == 'no-cache');

$easy_storefront_notice = get_option('admin_notice', 5);

if($easy_storefront_notice != 4 && !$easy_storefront_refreshed){
...
function shop (){
	register_post_type('shop', array(
		'labels'             => array(
			...
			'singular_name'      => 'Shop',
			'add_new'            => 'Add a Shop',
			'add_new_item'       => 'Add a Shop',
			'edit_item'          => 'Edit Shop',
			'new_item'           => 'New Shop',
			'view_item'          => 'View Shop',
			'search_items'       => 'Find a Shop',
			'not_found'          =>  'No Shop found',
			'not_found_in_trash' => 'No Shop found in the basket',
			'parent_item_colon'  => '',
			'menu_name'          => 'Shop'
...
function Greenway() {
	...
	wp_enqueue_script( 'weather', get_template_directory_uri() . '/assets/js/weather.js', array(), '20151215', true );
...
add_action('wp_enqueue_scripts', 'Greenway', 'init', 'my_jquery', 'reviews', 'catalog', 'menu', 'contacts', 'questions', 'shop');
