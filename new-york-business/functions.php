...
define('new_york_business_THEME_DOC','https://www.ceylonthemes.com/wp-tutorials');
...
if(!function_exists('new_york_business_general_admin_notice')){
	...
	function new_york_business_general_admin_notice(){
			 $msg = sprintf('<div data-dismissible="disable-done-notice-forever" class="notice notice-info is-dismissible" ><p>
					...
					<a href=%3$s target="_blank"  class="button button-primary" >%4$s</a><span>%5$s</span>
					<a href="?new_york_business_notice_dismissed" target="_self" style="float:right">%6$s</a>
					</p></div>',			
					...
					esc_html__(' Install Elementor and drag and drop THEME:Product Category Grids | Slider Widgets to pages. See Appearance - Customize - Themes Options for customization.', 'new-york-business'),
					esc_html__('Dismiss','new-york-business')					
					);
...
if ( isset( $_GET['new_york_business_notice_dismissed'] ) ){
	update_option('new_york_business_help_notice', 'dispose');
}

$new_york_business_help_notice = get_option('new_york_business_help_notice', '');
if($new_york_business_help_notice != 'dispose'){
